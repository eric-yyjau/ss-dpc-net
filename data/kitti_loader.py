...
class KittiLoaderPytorch(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        for s, i in zip(seq, range(0, len(seq))):
            ...
            print(
                os.path.join(
                    basedir, seq_name[s], "{}_data.mat".format(config["estimator_type"])
                )
            )
            data = sio.loadmat(
                os.path.join(
            ...
            if self.config["load_from_txt"] is not None:
                file = f"{basedir}/{seq_name[s]}/{self.config['load_from_txt']}"
                print(f"load from {file}")

                def kitti_to_mat(file):
                    est_poses = np.genfromtxt(file, delimiter=" ")
                    row = np.array([0, 0, 0, 1])
                    est_poses = [np.concatenate((p, row), axis=0) for p in est_poses]
                    est_poses = np.array(est_poses).reshape(-1, 16)
                    est_poses = est_poses.reshape(-1, 4, 4)
                    return est_poses

                poses = kitti_to_mat(file)
                print(f"{poses.shape}")
                self.raw_vo_traj.append(np.copy(poses))
            else:
                print(f"{np.copy(data['sparse_vo']).shape}")
                self.raw_vo_traj.append(np.copy(data["sparse_vo"]))
            ...
            if self.config["correction_rate"] != 1:
                ...
                self.raw_intrinsic_trials[i] = np.copy(
                    self.raw_intrinsic_trials[i][::cr]
                )
            ...
            if self.num_frames:
                self.left_cam_filenames[i] = np.copy(
                    self.left_cam_filenames[i][: self.num_frames]
                )
                self.right_cam_filenames[i] = np.copy(
                    self.right_cam_filenames[i][: self.num_frames]
                )
                self.raw_intrinsic_trials[i] = np.copy(
                    self.raw_intrinsic_trials[i][: self.num_frames]
                )
                self.raw_gt_trials[i] = np.copy(
                    self.raw_gt_trials[i][: self.num_frames]
                )
            ...
            for s, i in zip(seq, range(0, len(seq))):
                data = sio.loadmat(
                    os.path.join(
                        basedir,
                        seq_name[s],
                        "{}_data.mat".format(config["estimator_type"]),
                    )
                )
                self.left_cam_filenames.append(
                    np.copy(data["cam_02"].reshape((-1, 1)))[:: (cr + 1)]
                )
                self.right_cam_filenames.append(
                    np.copy(data["cam_03"].reshape((-1, 1)))[:: (cr + 1)]
                )
                self.raw_intrinsic_trials.append(
                    np.copy(data["intrinsics"])[:: (cr + 1)]
                )
        ...
        if self.augment_backwards:
            for s, i in zip(seq, range(0, len(seq))):
                self.left_cam_filenames.append(
                    np.flip(np.copy(self.left_cam_filenames[i]), axis=0)
                )
                self.right_cam_filenames.append(
                    np.flip(np.copy(self.right_cam_filenames[i]), axis=0)
                )
                self.raw_intrinsic_trials.append(
                    np.flip(np.copy(self.raw_intrinsic_trials[i]), axis=0)
                )
                self.raw_gt_trials.append(
                    np.flip(np.copy(self.raw_gt_trials[i]), axis=0)
                )
                self.raw_vo_traj.append(np.flip(np.copy(self.raw_vo_traj[i]), axis=0))

        ###         Merge data from all trials
        self.gt_samples, self.left_img_samples, self.right_img_samples, self.intrinsic_samples, self.vo_samples, = (
            self.reshape_data()
        )

        if self.skip:
            skip = self.config["skip"]
            self.gt_samples, self.left_img_samples, self.intrinsic_samples = (
                self.gt_samples[::skip],
                self.left_img_samples[::skip],
                self.intrinsic_samples[::skip],
            )
    ...
    def __getitem__(self, idx):
        ...
        if self.transform_img != None:
            imgs, intrinsics, gt_lie_alg = self.transform_img(
                imgs, intrinsics, gt_lie_alg
            )
        if self.config["use_flow"]:
            for i in range(0, len(imgs) - 1):
                ...
                flow_img = cv2.calcOpticalFlowFarneback(
                    flow_img1, flow_img2, None, 0.5, 3, 15, 3, 5, 1.2, 0
                )
                flow_img = torch.from_numpy(np.transpose(flow_img, (2, 0, 1))).float()
                orig_imgs = [
                    torch.from_numpy(np.transpose(im, (2, 0, 1))).float() / 255
                    for im in orig_imgs
                ]
                imgs.append(flow_img)
                imgs.append(
                    orig_imgs[0]
                )  # stack regular imgs at end for loss evaluation (keeps things consistent)
    ...
    def compute_target(self, idx):
        ...
        if self.config["estimator_type"] == "mono":
            if self.config["scale"] and np.linalg.norm(vo_lie_alg[0:3]) >= 1e-8:
    ...
    def reshape_data(self):
        ...
        gt_samples = self.raw_gt_trials[0][0 : self.seq_len, :, :].reshape(
            (1, self.seq_len, 4, 4)
        )
        vo_samples = self.raw_vo_traj[0][0 : self.seq_len, :, :].reshape(
            (1, self.seq_len, 4, 4)
        )
        intrinsic_samples = self.raw_intrinsic_trials[0][
            0 : self.seq_len, :, :
        ].reshape((1, self.seq_len, 3, 3))
        left_img_samples = self.left_cam_filenames[0][0 : self.seq_len].reshape(
            (1, self.seq_len, 1)
        )
        right_img_samples = self.right_cam_filenames[0][0 : self.seq_len].reshape(
            (1, self.seq_len, 1)
        )
        pose_per_sample = []
        ...
        for im in self.left_cam_filenames:
            ...
            left_img_samples = np.vstack(
                (left_img_samples, new_imgs[0 : pose_per_sample[count]])
            )  # get rid of extra imgs due to rounding of gt
            count += 1
        ...
        for im in self.right_cam_filenames:
            ...
            if self.samples_per_file == None:
                num_samples = int(im.shape[0] / (self.seq_len))
            new_imgs = self.split_data(im, num_samples, self.config["img_per_sample"])
            right_img_samples = np.vstack(
                (right_img_samples, new_imgs[0 : pose_per_sample[count]])
            )  # get rid of extra imgs due to rounding of gt
            count += 1
    ...
    def split_data(self, data, num_samples, sample_size):
        ...
        while i <= (data.shape[0] - sample_size):
            output = np.vstack(
                (output, data[i : i + samplesize].reshape(1, samplesize, data.shape[1]))
            )
            i += samplesize - 1
        return output[1:]
